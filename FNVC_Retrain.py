import argparse
from FNVC_utils import load_from_pickle, save_to_pickle, readData, printx, time, path
...
parser.add_argument(
    '--workpath', type=str,
    default='./',
    help='the location where all the tmp files and reult files will be saving, default current direction')

args = parser.parse_args()

workpath = args.workpath
train_TP_file = path.join(workpath, args.in_tp)
train_FP_file = path.join(workpath, args.in_fp)
premodel_file = args.model
outmodel_file = args.out_file
...
if premodel_file != 'no':
    ...
    xgb_params['xgb_model'] = list(premodel[0].values())[0]
